...
//   vector<p_match> & match = p_matched_2;
//   vector<KeyPoint> ipts;
//   ipts.resize(match.size());
//   for(int i=0; i<ipts.size(); ++i)
//   {
//       ipts[i] = KeyPoint(Point2f(match[i].u1c,match[i].v1c), 0);
//   }
//   Mat tmp;
//   Mat left = Mat(dims_c[1] , dims_c[0] , CV_8UC1, I1c, dims_c[2] );
//   drawKeypoints(left, ipts, tmp);
//   imshow("tmp",tmp);
//   waitKey(1);
}
 ...
 * @param i1        index1
 * @param m2        maxima2
 * @param step_size step_size between <Matcher::maximum>
 * @param k2        indexVector2
 * @param u_bin_num number of bin in u-axis
 * @param v_bin_num number of bin in v-axis
 * @param stat_bin
 * @param min_ind   correspondence's index
 * @param stage     0,1,2,3 this is a loop
 * @param flow      if false��use stereo search => constrain to 1d
 * @param use_prior [description]
 * @param u_        [description]
 * @param v_        [description]
 */
    ...
    for (int32_t v_bin=v_bin_min; v_bin<=v_bin_max; v_bin++) {
      ...
      for (vector<int32_t>::const_iterator i2_it=k2[k2_ind].begin(); i2_it!=k2[k2_ind].end(); i2_it++) {
        ...
        if (u2>=u_min && u2<=u_max && v2>=v_min && v2<=v_max) {
          ...
          xmm3 = _mm_sad_epu8 (xmm1,xmm3); // SAD
          xmm4 = _mm_sad_epu8 (xmm2,xmm4); // SAD
...
void Matcher::matching (int32_t *m1p,int32_t *m2p,int32_t *m1c,int32_t *m2c,
  ...
  if (method==0) {
    ...
    for (i1p=0; i1p<n1p; i1p++) {
      ...
      // match in circle
      findMatch(m1p,i1p,m2p,step_size,k2p,u_bin_num,v_bin_num,stat_bin,i2p, 0,false,use_prior); // ǰһ֡����ͼƥ��
      ...
      if (Tr_delta) {
        ...
        findMatch(m2p,i2p,m2c,step_size,k2c,u_bin_num,v_bin_num,stat_bin,i2c, 1,true ,use_prior); // ǰ��֡��ͼƥ��
      }

      findMatch(m2c,i2c,m1c,step_size,k1c,u_bin_num,v_bin_num,stat_bin,i1c, 2,false,use_prior); // ��ǰ֡����ͼƥ��

      if (Tr_delta)
        findMatch(m1c,i1c,m1p,step_size,k1p,u_bin_num,v_bin_num,stat_bin,i1p2,3,true ,use_prior,u1p,v1p);
      else
        findMatch(m1c,i1c,m1p,step_size,k1p,u_bin_num,v_bin_num,stat_bin,i1p2,3,true ,use_prior); // ǰ��֡��ͼƥ��

      // circle closure success? �Ƿ���match���гɱջ�?
      if (i1p2==i1p) {
        ...
        // if disparities are positive ͬһ֡��ƥ��㣬��ͼ�ĺ����������ͼ��
        if (u1p>=u2p && u1c>=u2c) {
...
void Matcher::removeOutliers (vector<Matcher::p_match> &p_matched,int32_t method) {
  ...
  // do we have enough points for outlier removal? //FIXED <= 50 maybe ok
  if (p_matched.size()<=50)
    return;
  ...
  // and fill triangle point vector for delaunay triangulation ���ǻ�
