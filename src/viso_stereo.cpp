...
void VisualOdometryStereo::GetPointsMap(std::vector<Eigen::Vector3f>& mvPoints,bool fuse)
{
  vector<Matcher::p_match> & match = matcher->p_matched_3;
  int sz = match.size();
  ...
  for(int i=0;i<sz;i++)
  {
      //double d = max(match[i].u1p - match[i].u2p, 0.0001f);
      ...
      {
        Eigen::Vector3f XYZ1c;

        float disp = max(match[i].u1c - match[i].u2c, 0.0001f);

        XYZ1c << float((match[i].u1c-param.calib.cu)*param.base/disp),
                float((match[i].v1c-param.calib.cv)*param.base/disp),
                float(param.calib.f*param.base/disp);

        mvPoints.push_back(XYZ1c);
      }
